...
import ..System
struct MKAbsynProgramTraverser
    ...
    function bundleElementsFromIterator(iter::Base.Generator{Base.Iterators.ProductIterator{Y},Z})
        join(list(iter), "\n")
    end


    function translateProgram(inProgram::MKAbsyn.Program)::String
        @match inProgram begin
            _ => begin
                bundleElementsFromIterator(translateClass(c) for c in inProgram.classes)
            end
        end
    end

    function translateClass(inClass::MKAbsyn.Class)::String
        ...
        @match inClass begin
            MKAbsyn.CLASS(
                ...
                classDef = translateClassDef(cBody, cInfo, cRestriction)
                restStr = begin
    ...
    function translateClassdef(
        ...
        @match inClassDef begin
            ...
            MKAbsyn.PARTS(
                ...
                bundleElementsFromIterator(translateClassPart(c) for c in dClassParts)
            end
            #TODO: OTHERS 
        end
    ...
    function translateClassPart(inClassPart::MKAbsyn.ClassPart)::String
        local cContents::List{Exp}
        ...
        @match inClassPart begin
            ...
            MKAbsyn.EQUATIONS(
                ...
                "equation\n" bundleElementsFromIterator(translateEquation(c) for c in cContents)
            end
            #TODO: OTHERS 
        end
    end

    function translateEquationItem(equation::MKAbsyn.EquationItem)::String
        local cEquation #= equation =#::Equation
        local cComment #= comment =#::Option{Comment}
        local cInfo #= line number =#::Info
        @match equation begin
            ...
            #TODO: add comment?
            #TODO: add info
            translateEquation(cEquation)
            end
        end
    end

    function translateEquation(equation::MKAbsyn.Equation)::String
        ...
        @match equation begin
            MKAbsyn.EQ_EQUALS(
                ...
                translateExpression(eLeftSide) + " = " + translateExpression(eRightSide)
            end

        end
    end

    function translateExpression(exp::Exp)
        @match exp begin
            MKAbsyn.INTEGER(
                value = eValue
            ) => begin
